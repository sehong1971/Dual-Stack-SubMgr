...
import (
	...
	"gerrit.o-ran-sc.org/r/ric-plt/submgr/pkg/ricapi"
)
...
type Control struct {
	*xapp.RMRClient
	...
	// START: grpc update
	// gRPC 동기화 처리를 위한 필드 추가
	grpcWaiters     map[string]chan *ricapi.E2SubscriptionOutMsg
	grpcWaitersLock sync.RWMutex
	// END: grpc update
}
...
// START: grpc update
// Waiter 관리 Helper 함수 구현
// RegisterGRPCWaiter: 새로운 gRPC 요청에 대한 응답 채널을 등록합니다.
func (c *Control) RegisterGRPCWaiter(grpcSubId string, resChan chan *ricapi.E2SubscriptionOutMsg) {
	c.grpcWaitersLock.Lock()
	defer c.grpcWaitersLock.Unlock()
	c.grpcWaiters[grpcSubId] = resChan
	xapp.Logger.Debug("gRPC Waiter registered for restSubId: %s", grpcSubId)
}

// UnregisterGRPCWaiter: 처리가 끝난(성공, 실패, 타임아웃 등) 대기자를 삭제합니다.
func (c *Control) UnregisterGRPCWaiter(grpcSubId string) {
	c.grpcWaitersLock.Lock()
	defer c.grpcWaitersLock.Unlock()
	delete(c.grpcWaiters, grpcSubId)
	xapp.Logger.Debug("gRPC Waiter unregistered for restSubId: %s", grpcSubId)
}

// GetGRPCWaiter: 특정 restSubId에 대해 기다리고 있는 채널이 있는지 확인합니다.
func (c *Control) GetGRPCWaiter(grpcSubId string) (chan *ricapi.E2SubscriptionOutMsg, bool) {
	c.grpcWaitersLock.RLock() // 읽기 잠금만 수행하여 성능 최적화
	defer c.grpcWaitersLock.RUnlock()
	ch, ok := c.grpcWaiters[grpcSubId]
	return ch, ok
}
...
func NewControl() *Control {
	...
	c := &Control{
		...
		LoggerLevel:        1,
		// START: grpc update
		grpcWaiters: make(map[string]chan *ricapi.E2SubscriptionOutMsg),
		// END: grpc update
	}
	...
	// START: grpc update
	// [신규 추가] gRPC 서버를 별도의 고루틴으로 실행하여 듀얼 스택 활성화
	xapp.Logger.Info("calling RunGRPCServer .....")
	go c.RunGRPCServer("50051")
	// END: grpc update
...
func (c *Control) processSubscriptionRequests(restSubscription *RESTSubscription, subReqList *e2ap.SubscriptionRequestList,
	clientEndpoint *models.SubscriptionParamsClientEndpoint, meid *string, dualSubId *string, xAppRmrEndpoint string, md5sum string, e2SubscriptionDirectives *E2SubscriptionDirectives) {
	...
	defer c.restDuplicateCtrl.SetMd5sumFromLastOkRequest(*dualSubId, md5sum)
	...
	for index := 0; index < len(subReqList.E2APSubscriptionRequests); index++ {
		...
		trans := c.tracker.NewXappTransaction(xapp.NewRmrEndpoint(xAppRmrEndpoint), *dualSubId, subReqMsg.RequestId, &xapp.RMRMeid{RanName: *meid})
		if trans == nil {
			...
			c.sendUnsuccesfullResponseNotification(dualSubId, restSubscription, xAppEventInstanceID, err, clientEndpoint, trans, errorInfo)
			continue
		}

		xapp.Logger.Debug("Handle SubscriptionRequest index=%v, %s", index, idstring(nil, trans))

		subRespMsg, errorInfo, err := c.handleSubscriptionRequest(trans, &subReqMsg, meid, *dualSubId, e2SubscriptionDirectives)

		// START: grpc update
		// [추가] gRPC 대기자가 있는지 확인
		waiterChan, isGrpc := c.GetGRPCWaiter(*dualSubId)
		// END: grpc update
		...
		if err != nil {
			...
			// [추가] gRPC 실패 응답 전송
			if isGrpc {
				waiterChan <- c.buildGRPCFailureResponse(err, restSubscription)
			} else {
				// REST: 기존 방식대로 HTTP Notification 전송
				c.sendUnsuccesfullResponseNotification(dualSubId, restSubscription, xAppEventInstanceID, err, clientEndpoint, trans, errorInfo)
			}
			// END: grpc update
		} else {
			...
			xapp.Logger.Debug("SubscriptionRequest index=%v processed successfullyfor %s. endpoint=%v:%v, XappEventInstanceID=%v, E2EventInstanceID=%v, %s",
				index, *dualSubId, clientEndpoint.Host, *clientEndpoint.HTTPPort, xAppEventInstanceID, e2EventInstanceID, idstring(nil, trans))

			// START: grpc update
			// [추가] gRPC 성공 응답 전송
			if isGrpc {
				waiterChan <- c.buildGRPCSuccessResponse(subRespMsg, *dualSubId, restSubscription)
			} else {
				c.sendSuccesfullResponseNotification(dualSubId, restSubscription, xAppEventInstanceID, e2EventInstanceID, clientEndpoint, trans, errorInfo)
			}
			// END: grpc update
		}
	}
...
// START: grpc update
// buildGRPCSuccessResponse: E2 Node의 응답(subRespMsg)을 gRPC Success 메시지로 변환
// buildGRPCSuccessResponse 수정안
func (c *Control) buildGRPCSuccessResponse(subRespMsg *e2ap.E2APSubscriptionResponse, restSubId string, restSub *RESTSubscription) *ricapi.E2SubscriptionOutMsg {

	xapp.Logger.Debug("Building gRPC Success Response for restSubId: %s", restSubId)

	// 1. 수락된 액션 리스트 (Admitted List) 매핑
	admittedList := []*ricapi.E2SubscriptionSuccess_E2ActionAdmittedItem{}
	for _, item := range subRespMsg.ActionAdmittedList.Items {
		// WG3 규격에 따라 요청된 ID와 수락된 ID를 모두 설정합니다.
		actionID := uint32(item.ActionId)
		admittedItem := &ricapi.E2SubscriptionSuccess_E2ActionAdmittedItem{
			RequestedE2ActionId: &ricapi.E2ActionId{
				Value: &actionID,
			},
			E2ActionId: &ricapi.E2ActionId{
				Value: &actionID,
			},
		}
		admittedList = append(admittedList, admittedItem)
	}

	// 2. 거절된 액션 리스트 (Not Admitted List) 매핑
	notAdmittedList := []*ricapi.E2SubscriptionSuccess_E2ActionNotAdmittedItem{}
	for _, item := range subRespMsg.ActionNotAdmittedList.Items {
		actionID := uint32(item.ActionId)
		notAdmittedList = append(notAdmittedList, &ricapi.E2SubscriptionSuccess_E2ActionNotAdmittedItem{
			RequestedE2ActionId: &ricapi.E2ActionId{
				Value: &actionID,
			},
			// 필요 시 Failure Cause 매핑 추가
		})
	}

	// 3. 최종 gRPC 메시지 조립 (Oneof 구조 반영)
	// subRespMsg.RequestId.InstanceId 등을 uint32로 변환 후 헬퍼 사용
	instanceID := uint32(subRespMsg.RequestId.InstanceId)
	funcID := uint32(subRespMsg.FunctionId)

	return &ricapi.E2SubscriptionOutMsg{
		TypeOfMessage: &ricapi.E2SubscriptionOutMsg_E2SubscriptionSuccess{
			E2SubscriptionSuccess: &ricapi.E2SubscriptionSuccess{
				ProcedureTransactionId: restSub.PtransID,

				E2RequestId: &ricapi.E2RequestId{
					Value: &instanceID, // *uint32 타입에 맞게 주소 전달
				},

				RanFunctionId: &ricapi.RanFunctionId{Value: &funcID},

				E2ActionsAdmittedList:    admittedList,
				E2ActionsNotAdmittedList: notAdmittedList,
			},
		},
	}
}

func (c *Control) toPtr(v uint32) *uint32 {
	return &v
}

// buildGRPCFailureResponse: 내부 에러를 gRPC 규격의 Failure 메시지로 변환
func (c *Control) buildGRPCFailureResponse(err error, restSub *RESTSubscription) *ricapi.E2SubscriptionOutMsg {

	xapp.Logger.Error("Building gRPC Failure Response for restSubId: %s, error: %v", restSub.Meid, err)

	// 1. Go error를 gRPC 규격의 Cause 구조체로 매핑
	ricCause := c.mapErrorToRicapiCause(err)

	// 2. 최종 Failure 메시지 조립 (Oneof 구조 반영)
	return &ricapi.E2SubscriptionOutMsg{
		TypeOfMessage: &ricapi.E2SubscriptionOutMsg_E2SubscriptionFailure{
			E2SubscriptionFailure: &ricapi.E2SubscriptionFailure{
				// gRPC 요청 수신 시 보관했던 트랜잭션 ID 복원
				ProcedureTransactionId: restSub.PtransID,

				// 에러 원인 설정
				Cause: ricCause,

				// 필요 시 RAN Function ID 등 추가 정보 매핑
				RanFunctionId: &ricapi.RanFunctionId{
					Value: c.toPtr(0), // 기본값 또는 실제 ID
				},
			},
		},
	}
...
// mapErrorToRicapiCause: 에러 메시지 패턴에 따라 카테고리별 Cause 생성
func (c *Control) mapErrorToRicapiCause(err error) *ricapi.Cause {
	errStr := strings.ToLower(err.Error())

	// 1. E2AP 상세 원인 메시지 생성
	innerE2ApCause := &ricapi.Cause_E2ApCause{}

	// 에러 키워드 분석 및 매핑
	if strings.Contains(errStr, "timeout") || strings.Contains(errStr, "timer") {
		// Protocol 그룹 매핑
		innerE2ApCause.CauseGroup = &ricapi.Cause_E2ApCause_Protocol_{
			Protocol: ricapi.Cause_E2ApCause_PROTOCOL_UNSPECIFIED,
		}
	} else if strings.Contains(errStr, "connection") || strings.Contains(errStr, "unreachable") {
		// [확인된 정보 반영] Transport 그룹 매핑
		// 상수명: Cause_E2ApCause_TRANSPORT_UNSPECIFIED
		innerE2ApCause.CauseGroup = &ricapi.Cause_E2ApCause_Transport_{
			Transport: ricapi.Cause_E2ApCause_TRANSPORT_UNSPECIFIED,
		}
	} else if strings.Contains(errStr, "syntax") {
		// Protocol 구문 에러
		innerE2ApCause.CauseGroup = &ricapi.Cause_E2ApCause_Protocol_{
			Protocol: ricapi.Cause_E2ApCause_TRANSFER_SYNTAX_ERROR,
		}
	} else {
		// 기본값: Misc 그룹 (패턴에 따라 상수명 추정)
		innerE2ApCause.CauseGroup = &ricapi.Cause_E2ApCause_Misc_{
			Misc: ricapi.Cause_E2ApCause_MISC_UNSPECIFIED,
		}
	}

	// 2. 최상위 Cause 구조체 조립
	return &ricapi.Cause{
		CauseGroup: &ricapi.Cause_E2ApCause_{
			E2ApCause: innerE2ApCause,
		},
	}
}
